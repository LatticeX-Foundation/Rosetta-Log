...
void Logger::set_filename(const std::string& filename, const std::string& task_id) {
	...
	if (task_id.empty()) {
		if (spdlog::get(SPDLOG_ROSETTA_LOGGER_NAME)) {
			...
			auto logger = spdlog::basic_logger_mt(filename.c_str(), filename.c_str());
			...
			auto logger = spdlog::basic_logger_mt(SPDLOG_ROSETTA_LOGGER_NAME, filename.c_str());
		...
		auto logger = spdlog::basic_logger_mt(task_id.c_str(), filename.c_str());
